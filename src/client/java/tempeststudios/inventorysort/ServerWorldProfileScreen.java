...
public class ServerWorldProfileScreen extends Screen {
    ...
    public void render(GuiGraphics g, int mouseX, int mouseY, float partialTick) {
        ...
        if (profileBox != null && profileBox.getValue().isEmpty()) {
            g.drawString(this.font, "Type a world name...", panelX + PAD + 6, fieldY + 5, InvUi.TEXT_DIM, false);
        }
        ...
        for (int i = 0; i < shown; i++) {
            ...
            g.drawString(this.font, truncate(profile, rowsW - 70), rowsX + 8, y + 5,
            ...
            if (!status.isEmpty()) {
                g.drawString(this.font, status, rowsX + rowsW - 8 - this.font.width(status), y + 5, statusColor, false);
            }
        }
        if (shown == 0) {
            g.drawString(this.font, "No saved worlds yet.", rowsX + 6, listY + 8, InvUi.TEXT_MUTED, false);
        }
        g.disableScissor();
        ...
        if (hintH > 0) {
            g.drawString(this.font, "Press Confirm to start tracking this world.",
